...
Adafruit_MQTT_Publish ownerDetail = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/Owner_detail");
Adafruit_MQTT_Publish taxCollected = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/Toll_collected");
Adafruit_MQTT_Publish toll_location = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/Location");
...
void loop() {
  ...
  if (irInput == LOW ){
    ...
    if (tag == "195196235253"){
      ...
      scanCard(amt53, "Ma'am");  //calling the scanCard function to display the message

      //Publishing the value on the Adafruit
      ownerDetail.publish("Miss Bulbul");
      taxCollected.publish(largeVehicle);
      toll_location.publish("SJCE MYSORE");
    }
    else if(tag == "5111085245"){
      vehicleType = "medium";
      amt45 = deductTax(amt45, vehicleType);     //Deducting the tax amount
      scanCard(amt45, "Sir");  //calling the scanCard function

      //Publishing the value on the Adafruit
      ownerDetail.publish("Mr. Charlie");
      taxCollected.publish(mediumVehicle);
      toll_location.publish("SJCE MYSORE");
    }
    else if(tag == "137111149124"){
      vehicleType = "small";
      amt24 = deductTax(amt24, vehicleType);      //Deducting the amount
      scanCard(amt24, "Sir");  //calling the scanCard function to display the message

      //Publishing the value on the Adafruit
      ownerDetail.publish("Mr. Bean");
      taxCollected.publish(smallVehicle);
      toll_location.publish("SJCE MYSORE");
    }
    else{
      display.clearDisplay();
